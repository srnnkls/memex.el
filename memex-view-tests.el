...
(ert-deftest memex-view-asks-for-a-kind-and-puts-it-back-at-one-key ()
  "The three states are worth a menu; asking for a kind and putting it
back as it opened is worth a single key."
  (skip-unless (featurep 'magit-section))
  (unwind-protect
      (let ((buffer (memex-view-tests--open (memex-view-tests--records)
                                            memex-view-tests--session-id
                                            memex-view-tests--source-path)))
        (with-current-buffer buffer
          (should (eq (memex-view--state 'tool) 'collapse))
          (memex-view-toggle-tool)
          (should (eq (memex-view--state 'tool) 'show))
          (memex-view-toggle-tool)
          (should (eq (memex-view--state 'tool) 'collapse))
          (memex-view-toggle-system)
          (should (eq (memex-view--state 'system) 'show))
          (should-not (memq 'system buffer-invisibility-spec))
          (memex-view-toggle-system)
          (should (eq (memex-view--state 'system) 'hide))
          (memex-view-toggle-assistant)
          (should (eq (memex-view--state 'assistant) 'hide))
          (should (invisible-p (memex-view-tests--position-of
                                "second line of beta")))
          (memex-view-toggle-assistant)
          (should (eq (memex-view--state 'assistant) 'show))
